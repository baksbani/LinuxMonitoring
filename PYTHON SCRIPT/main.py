    ...
    try:
        with open('/opt/linuxmonitor/serverauth.txt', 'r') as auth_file:
            ...
            if (response == "auth_result_working"):
                ...
                choice = raw_input('Do you want to configure your server auth code [Y/N]: ')
        ...
        choice = raw_input('Do you want to create and configure your server auth code [Y/N]: ')
        if (choice == "y") or (choice == "Y"):
            create_auth_file()
        elif (choice == "n") or (choice == "N"):
            print ("Missing Authentification Code error will stay")
...
def configure_auth_file():
        try:
            with open('/opt/linuxmonitor/serverauth.txt', 'r+') as auth_file:
                auth_file.truncate()
                your_auth = raw_input("Enter your server's authentification code: ")
                auth_file.write(your_auth)
                print ("-------------------------------------------------------------------")
                print ("#                  Your new auth code is saved.                   #")
                print ("-------------------------------------------------------------------")
...
def send_monitor_data():
    ...
    total_hdd = round((float(popen("df -P | awk '/^\/dev\/sd[a-z]/ { sum+=$2 } END { print sum }'").readline().strip())/pow(1024,2)),2)
    used_hdd = round((float(popen("df -P | awk '/^\/dev\/sd[a-z]/ { sum2+=$3 } END { print sum2 }'").readline().strip())/pow(1024,2)),2)
    available_hdd = round((float(popen("df -P | awk '/^\/dev\/sd[a-z]/ { sum3+=$4 } END { print sum3 }'").readline().strip())/pow(1024,2)),2)
